...
public class ParticuleA extends Particule {
    ...
    public boolean collisionSimple(List<Particule> c) {
        ...
        if (enCollisionFrontale.size() != 1) return false;
        else {
            Particule.collisionsSimplesTraitees.add(this);
            if (this.directionCourante > Math.PI) this.prochaineDirection = this.directionCourante - Math.PI;
            else this.prochaineDirection = this.directionCourante + Math.PI;

            for (Particule p : enCollisionFrontale) {
                if (p.directionCourante > Math.PI) p.prochaineDirection = p.directionCourante - Math.PI;
                else p.prochaineDirection = p.directionCourante + Math.PI;
