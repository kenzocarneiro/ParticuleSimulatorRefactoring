...
public class DistancesEtDirections {
    ...
    public static double directionDepuisUnPoint(Particule p, double xOrigine, double yOrigine, double axe) {
        ...
        double x = p.getX();
        double y = p.getY();
        if (x != xOrigine) angle = Math.atan((y - yOrigine) / (x - xOrigine));
        else if (y < yOrigine) angle = -(Math.PI / 2);
        else angle = (Math.PI / 2);
        // atan a deux solutions, donc il faut corriger...
        if (x < xOrigine) {
            if (y < yOrigine) angle -= Math.PI;
            else angle += Math.PI;
        }
        ...
        if (dir >= Math.PI) return dir - (Math.PI * 2);
        else if (dir < -Math.PI) return dir + (Math.PI * 2);
        else return dir;
    }
}
